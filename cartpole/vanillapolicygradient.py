...
def compute_policy_gradient(episode_rewards, episode_actions,
    ...
    for t in xrange(episode_length):
        # We compute the policy on the observation and theta.
        pi = policy(episode_observations[t], theta)
        action = episode_actions[t]
        vt = sum(episode_rewards[t::])
        if action == 0:
            grad_theta_log_pi = - pi[1] * episode_observations[t] * vt
        else:
            grad_theta_log_pi = (1-pi[1]) * episode_observations[t] * vt
...
theta = train_policy_gradient_agent(num_episodes, max_episode_length,
        batch_size, learning_rate, gamma, regularisation, reward_to_exit)
